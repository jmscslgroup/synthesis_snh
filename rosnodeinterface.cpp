...
// Generated 20-May-2021 02:34:53
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"synthesis_snh\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(synthesis_snh_M, (NULL));
        synthesis_snh_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(synthesis_snh_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(synthesis_snh_M)
                == (NULL)) || rtmGetStopRequested(synthesis_snh_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(synthesis_snh_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
